    ...
    <div className="flex flex-1 flex-col items-center justify-center bg-[#05070a] p-5">
      <div className="relative mb-6 h-32 w-32">
        <svg className="h-full w-full -rotate-90" viewBox="0 0 32 32">
          <circle cx="16" cy="16" fill="transparent" r="16" stroke="#27272a" strokeWidth="4" />
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="font-mono text-[8px] font-black tracking-[0.2em] text-zinc-500">TOTAL</span>
        </div>
      </div>

      {items.length === 0 ? (
        <div className="font-mono text-xs font-black uppercase tracking-[0.22em] text-zinc-600">NO DATA</div>
      ) : (
          ...
          {items.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-mono font-black text-zinc-100">{percent}%</span>
              </div>
            );
          })}
        </div>
    ...
    <main className="min-h-full space-y-6 bg-[#05070a] p-4 text-zinc-100 sm:p-6 md:p-8">
      ...
      <div className="relative grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard
          ...
          delay={320}
          value={getMetric(dashboard, ["pendingBookings", "pending", "waitingBookings"])}
        />
      </div>

      <div className="relative grid gap-4 xl:grid-cols-[minmax(0,1.8fr)_minmax(320px,1fr)]">
        <section className="admin-reveal flex h-[340px] flex-col overflow-hidden border border-zinc-800 bg-zinc-950 transition duration-300 hover:border-cyan-400/50" style={{ animationDelay: "380ms" }}>
          <div className="flex items-center justify-between border-b border-zinc-800 bg-black p-4">
            <span className="font-mono text-xs font-black uppercase tracking-[0.2em] text-zinc-300">DOANH THU 7 NGÀY QUA</span>
      ...
      <section className="admin-reveal relative flex flex-col overflow-hidden border border-zinc-800 bg-zinc-950" style={{ animationDelay: "540ms" }}>
        ...
        <div className="flex items-center justify-between border-b border-zinc-800 bg-black p-4">
          <span className="font-mono text-xs font-black uppercase tracking-[0.2em] text-zinc-300">GIAO DỊCH GẦN ĐÂY</span>
          <span className="font-mono text-[10px] font-black uppercase tracking-[0.2em] text-zinc-600">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left">
            ...
            <tbody className="font-mono text-[13px]">
              {bookings.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-10 text-center font-mono text-xs font-black uppercase tracking-[0.22em] text-zinc-600">
                    NO DATA
                  </td>
                </tr>
              ) : (
