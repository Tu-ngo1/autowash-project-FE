  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        ...
        "Đã xảy ra lỗi khi cấu hình vận hành cho ngày mai."
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl font-mono text-xs font-bold uppercase leading-6 tracking-[0.14em] text-zinc-500">
                Thiết lập lịch mở cửa, số khoang rửa và ca hoạt động áp dụng cho ngày mai: <span className="text-cyan-300 font-black">{tomorrowFormatted} ({tomorrowStr})</span>.
              </p>
            </div>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              {error && (
                <div className="border border-red-500/35 bg-red-950/20 p-4 font-mono text-xs text-red-400 flex items-start gap-3">
                  <span className="material-symbols-outlined text-[18px] text-red-400 mt-0.5">warning</span>
                  <div>
                    <div className="font-black uppercase tracking-wider mb-1">Cấu hình không hợp lệ</div>
                    <p>{error}</p>
                  </div>
                </div>
              )}
              ...
              {successData && (
                <div className="border border-emerald-500/35 bg-emerald-950/20 p-4 font-mono text-xs text-emerald-400 flex items-start gap-3">
                  <span className="material-symbols-outlined text-[18px] text-emerald-400 mt-0.5">check_circle</span>
                  <div>
                    <div className="font-black uppercase tracking-wider mb-1">Cập nhật thành công</div>
                    <p className="mb-2">Đã thiết lập cấu hình cho ngày <span className="font-black underline">{successData.configDate || tomorrowStr}</span>.</p>
                    <div className="space-y-1 pl-2 border-l border-emerald-500/30 text-emerald-300 font-bold uppercase tracking-wide">
                      ...
                      <div>Giờ đóng cửa tính toán: {successData.closeTime || "Chưa xác định"}</div>
                      <div>Số lượng khoang rửa: {successData.bayCount} khoang</div>
                      <div>Trạng thái hoạt động: {successData.isActive ? "ĐANG HOẠT ĐỘNG" : "TẮT"}</div>
                    </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    {timeOptions.map((time) => (
                      <option key={time} value={time} className="bg-black text-zinc-100">
                        {time}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <p className="mt-1 font-mono text-[10px] text-zinc-500 italic">
                    * Lưu ý: Mỗi ca làm việc kéo dài cố định 90 phút. Ví dụ: 6 ca từ 08:00 sẽ kết thúc vào lúc 17:00.
                  </p>
                </div>
          ...
          <div className="space-y-6">
            <div className="border border-zinc-800 bg-zinc-950 p-5">
              ...
              <div className="font-mono text-xs text-zinc-500 space-y-3 leading-relaxed">
                <p>
                  1. <strong className="text-zinc-300">Giờ mở cửa</strong>: Chọn thời gian tiệm bắt đầu đón những lượt khách rửa đầu tiên trong ngày mai.
                </p>
                <p>
                  2. <strong className="text-zinc-300">Số lượng ca</strong>: Xác định tổng số ca hoạt động liên tục trong ngày. Mỗi ca kéo dài đúng 90 phút. Hệ thống sẽ tự động tính toán ra <strong className="text-zinc-300">Giờ đóng cửa</strong>.
                </p>
                <p>
                  3. <strong className="text-zinc-300">Số khoang rửa</strong>: Số lượng khoang rửa (bay) thực tế phục vụ đồng thời. Càng nhiều khoang thì sức chứa đặt lịch cùng thời điểm càng tăng.
                </p>
                <div className="p-3 bg-cyan-400/5 border border-cyan-400/10 text-cyan-300/80">
                  Cấu hình này sẽ được áp dụng trực tiếp cho ngày mai. Vui lòng thống nhất với nhân viên vận hành trước khi cập nhật.
                </div>
              </div>
            </div>
          </div>
